...
if (($output = message()) !== null) {
  ...
  &&(!empty($_POST['email'])) && (!(trim($_POST['email']) == ""))){
   	...
   	$stmt3 = $mysqli->prepare($query3);
		...
		if($stmt3) {
			$_SESSION['message'] = "The user profile has been updated";
      redirect("OMSreadusers_admins.php");
		}else{
			$_SESSION["message"] = "Could not update the user profile! Fill in all information!";
			redirect("OMSreadusers_admins.php");
		}

	}else{
		$_SESSION["message"] = "Could not update the opportunity! Fill in all information!";
		redirect("OMSreadusers_admins.php");
	}
      ...
      echo "<form action='OMSupdate_userprofile_admins.php' method='POST'>\n";
